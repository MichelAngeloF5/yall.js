...
IMPORTANT
This is a fork from original project just including background images loading. You just have to include `data-src="path/to/image.jpg"` and the class `lazy-bg` to this image be set as `style="background-image:url(path/to/image.jpg)"`

```html
<div class="masthead lazy-bg" data-src="path/to/image.jpg"></div>
```
